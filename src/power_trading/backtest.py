...
class PlotlyVisualizer:
    ...
    def plot_backtest(cls, strategy, strategy_params, df, equity_curve):
        ...
        if strategy_params.get("strategy") == "moving_average":
            fast_ma = np.where(
                np.isnan(strategy.fast_ma.array), None, strategy.fast_ma.array
            )
            slow_ma = np.where(
                np.isnan(strategy.slow_ma.array), None, strategy.slow_ma.array
            )
...
def run_backtest(params):
    ...
    equity_curve = pd.Series(
        strategy.equity_curve, index=df.index[-len(strategy.equity_curve) :]
    )
